...
{
    public partial class ProcessSTA : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsCallback && !IsPostBack)
            {
                ...
                paramVarian = ModelRepository.RetrieveVariantIdByModelId(paramModel);
        ...
        {
            SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
        ...
        {
            GetdataSave_mPackingMonth = Convert.ToInt16(dtPackingMonth.Text);
        ...
        {
            GetdataSave_mPackingMonth = Convert.ToInt32(Request.QueryString["vpm"]);
            GetdataSave_mModelId = Convert.ToInt32(ModelId.SelectedItem.Value);
            GetdataSave_mVarianId = Convert.ToInt32(VarianId.SelectedItem.Value);
            ...
            //---Exec STA            
            string sqlSTA = string.Empty;
            sqlSTA = "exec GenerateSTA " + GetdataSave_mPackingMonth + ", " + GetdataSave_mModelId + ", " + GetdataSave_mVarianId + " ";

            SqlCommand ExecSTA = new SqlCommand();
            ExecSTA.Connection = conn;
            ExecSTA.CommandTimeout = 0;
            ExecSTA.CommandText = sqlSTA;
            string exeMsg = Convert.ToString(ExecSTA.ExecuteScalar());

            //SqlCommand ExecSTA = new SqlCommand(sqlSTA, conn);            
        }
    }
}