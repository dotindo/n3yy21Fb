...
namespace DotMercy.custom
{
    public partial class PackingList : System.Web.UI.Page
    {
        ...
        public static string GetdataSave_mPackingMonth = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsCallback && !IsPostBack)
            {
                ...
                //paramFileType = Convert.ToInt32(Request.QueryString["docType"]);                       

                dtPackingMonth.Text = Request.QueryString["vpm"].ToString();
                ModelId.Value = paramModel;
                VarianId.Value = ModelRepository.RetrieveVariantNameByModelIdAndVariantId(paramModel, paramVarian); ;              
                FileType.Value = FileTypeRepository.RetrieveFileTypeIdByName(Request.QueryString["docType"].ToTitleCase());

                //GetdataSave_mPackingMonthName = dtPackingMonth.Text;
                ////GetdataSave_mModelName = ModelId.SelectedItem.Value.ToString();
                //GetdataSave_mVarianName = VarianId.SelectedItem.Text;
                //GetdataSave_mFileTypeName = FileType.SelectedItem.Text;

                dtPackingMonth.Enabled = false;
                ModelId.Enabled = false;
                VarianId.Enabled = false;
        ...
        protected void UcDataUji_FileUploadComplete(object sender, FileUploadCompleteEventArgs e)
        {
            try
            {
                //GetdataSave_mPackingMonth = Convert.ToInt32(dtPackingMonth.Text);
                ...
                /////arf: varianid.selecteditemnull, jadi giniin dulu sementara
                //GetdataSave_mVarianId = Convert.ToInt16(Request.QueryString["variant"]);
                //GetdataSave_mFileType = Convert.ToInt16(FileType.SelectedItem.Value);

                //e.CallbackData = this.SavePostedFile(e.UploadedFile, _mPackingMonthName,_mModelName,_mVarianName,_mFileTypeName);
                ...
                string mPackingMonthId = GetdataSave_mPackingMonth;
                ...
                /*
                 * dipindah ke proses detail
                if (GetdataSave_mFileTypeName == "Assy PC Number")
                {
                    Process_SaveMaster_AssyPC(mModelId, mVarianId, mPackingMonthId, mFileType);
                } */
        ...
        string SavePostedFile(UploadedFile uploadedFile)
        {
            ...
            if (GetdataSave_mFileTypeName == "Assy Pc Number")
            {
                _ProcessExcel_AssyPCNumber(ext, resFileName);
            }
        ...
        private void _ProcessExcel(string ext, string fileXls)
        {
            ...
            string connString = "";
            string strFileType = ext.ToLower();
            string path = fileXls;
            ...
            if (strFileType.Trim() == ".TXT" || strFileType.Trim() == ".txt")
            {
                ...
                while ((line = File.ReadLine()) != null)
                {
                    ...
                    WeightPackage = WeightPackage.Replace(',', '.');
                    string VolPackage = Convert.ToString(line.Substring(311, 6)).Trim();
                    VolPackage = VolPackage.Replace(',', '.');
                    ...
                    //---cek data name Packing Month            
                    string sqlCek = string.Empty;
                    sqlCek = "select top 1 id from PackingLists where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                    SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                    selectCek.CommandTimeout = 0;
                    SqlDataReader RDCek = selectCek.ExecuteReader();

                    int IdHeaderCek = 0;
                    while (RDCek.Read())
                    {
                        IdHeaderCek = Convert.ToInt32(RDCek["Id"]);
                    }
                    RDCek.Close();
                    //-----------------------------end header---


                    int IdHeader = IdHeaderCek;
                    if (IdHeaderCek == 0)
                    {
                        cmd.CommandText = "insert into PackingLists (PackingMonth, ModelId, VarianId, FileType) " +
                                    " values (" + mPackingMonthId + "," + mModelId + ", " + mVarianId + ", " + mFileType + ")";

                        string exeMsg0 = Convert.ToString(cmd.ExecuteScalar());

                        //---get data name Packing Month            
                        string sqlPM = string.Empty;
                        sqlPM = "select SCOPE_IDENTITY() IdHeader";
                        ...
                        while (RDPM.Read())
                        {
                            IdHeader = Convert.ToInt32(RDPM["IdHeader"]);
                        }
                        RDPM.Close();
                    }
                    //-----------------------------end header---
                    
                    //----get varian from VehicleOrder
                    string sqlget = string.Empty;
                    sqlget = "select top 1 isnull(VariantId,0) VariantId from VehicleOrderDetails where ProdNumber='" + Productionnofrom + "' ";
                    SqlCommand selectGet = new SqlCommand(sqlget, conn);
                    selectGet.CommandTimeout = 0;
                    SqlDataReader RDGet = selectGet.ExecuteReader();

                    int VHVarianId = 0;
                    while (RDGet.Read())
                    {
                        VHVarianId = Convert.ToInt32(RDGet["VariantId"]);
                    }
                    RDGet.Close();
                    //-----------------------------end header---


                    cmd.CommandText = "insert into PackingListDetails (PackingListId, Identification, PackingCompany, PlantCode, Year, Month, Consignment, CountryCode, CountryDescription, Model, ModelDescription, " +
                                        " Productionnofrom, " +
                                        ...
                                        " Options, Lot, Counter1, Counter2, ConsigneeKey, EX33Equalization, PartNo, VariantId) " +
                                        " values (" + IdHeader + "," + Identification + ", '" + PackingCompany + "','" + PlantCode + "', " +
                                        ...
                                        " '" + Remark + "','" + ColourDescriptionGer + "','" + DolourDescriptionEng + "','" + AssemblyStation + "','" + PosWithinPackage + "','" + AppliModel + "','" + AppliAA + "','" + KGU + "','" + TU + "','" + POS + "','" + AppliTxtGer + "','" + AppliTxtEng + "','" + Packer + "', " +
                                        " '" + Options + "','" + Lot + "','" + Counter1 + "','" + Counter2 + "','" + ConsigneeKey + "','" + EX33Equalization + "','" + PartNo + "', " + VHVarianId  + ")";
                ...
                // Suspend the screen. --> arf: why?
                //Console.ReadLine();
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/PackingLists/list')", true);
            }
        ...
        private void _ProcessExcel_AssyPCNumber(string ext, string fileXls)
        {

            /*string extractPath = Server.MapPath(fileXls);
            using (ZipFile zip = ZipFile.Read(UcDataUji.PostedFile.InputStream))
            {
            ...
            if (strFileType.Trim() == ".TXT" || strFileType.Trim() == ".txt")
            {
                ...
                while ((line = File.ReadLine()) != null)
                {
                    ...
                    string CountryCode = Convert.ToString(line.Substring(10, 3));
                    string BauMuster = Convert.ToString(line.Substring(13, 6)).Trim();
                    string PackingMonth = Convert.ToString(line.Substring(21, 6)).Trim();
                    string PcNumber = Convert.ToString(line.Substring(28, 21)).Trim();
                    string Description = Convert.ToString(line.Substring(40, 58)).Trim();
                    string PartNumber = Convert.ToString(line.Substring(98, 17)).Trim();
                    string ES2 = Convert.ToString(line.Substring(107, 7)).Trim();
                    string Description2 = Convert.ToString(line.Substring(114, 54)).Trim();

                    string DialogAddress1 = Convert.ToString(line.Substring(175, 7)).Trim();
                    string DialogAddress2 = Convert.ToString(line.Substring(182, 6)).Trim();
                    string DialogAddress3 = Convert.ToString(line.Substring(188, 6)).Trim();
                    string DialogAddress4 = Convert.ToString(line.Substring(194, 4)).Trim();
                    string DialogAddress5 = Convert.ToString(line.Substring(199, 6)).Trim();

                    string DialogAddress = DialogAddress1 + "." + DialogAddress3 + "."
                                           + DialogAddress4 + "." + DialogAddress5;


                    string AA = DialogAddress2;  //Convert.ToString(line.Substring(182, 6)).Trim();
                    string Qty0 = Convert.ToString(line.Substring(205, 6)).Trim();
                    string[] Qty1 = Qty0.Split(',');                    
                    string Qty = Qty1[0];
                    string ProductionNo = Convert.ToString(line.Substring(292, 8)).Trim();
                    string PackId = Convert.ToString(line.Substring(300, 6)).Trim();
                    ...
                    //---cek data name Packing Month            
                    string sqlCek = string.Empty;
                    sqlCek = "select top 1 id from AssyPCNumber where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                    SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                    selectCek.CommandTimeout = 0;
                    SqlDataReader RDCek = selectCek.ExecuteReader();
                    ...
                    if (IdHeaderCek == 0)
                    {
                        cmd.CommandText = "insert into AssyPCNumber (PackingMonth, ModelId, VarianId, FileType) " +
                                        " values (" + mPackingMonthId + "," + mModelId + ", " + mVarianId + ", " + mFileType + ")";
                    ...
                    cmd.CommandText = "insert into AssyPCNumberDetail (AssyPCNumberId, CountryCode, BauMuster, PackingMonth, PcNumber, Description, PartNumber, ES2, Description2, DialogAddress, AA, Qty, " +
                                        " ProductionNo, PackId) " +
                                        " values (" + IdHeader + ",'" + CountryCode + "','" + BauMuster + "','" + PackingMonth + "','" + PcNumber + "','" + Description + "','" + PartNumber + "', " +
                                        " '" + ES2 + "','" + Description2 + "','" + DialogAddress + "','" + AA + "'," + Convert.ToInt32(Qty) + ", '" + ProductionNo + "','" + PackId + "')";
                ...
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/AssyPCNumber/list')", true);
        ...
        private void Import_Proses_alteration(string strValue, int __no)
        {
            ...
            try
            {
                ...
                //---cek data name Packing Month            
                string sqlCek = string.Empty;
                sqlCek = "select top 1 id from Alterations where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                selectCek.CommandTimeout = 0;
                SqlDataReader RDCek = selectCek.ExecuteReader();
                ...
                if (Pem != "")
                {
                    cmd.CommandText = "insert into AlterationDetails (AlterationId, VpMonat, Pem, PemModelLine, PartList, Aa, Application, KennzAltNeu, PartNo, Es1, " +
                                        " IndexNo, Description, Piece, Code, Remark ) " +
                                        " values (" + IdHeader + "," + VpMonat + ", '" + Pem + "', '" + PemModelLine + "','" + PartList + "', " +
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/Alterations/list')", true);
            }
        ...
        private void Import_Proses_dialog(string strValue, int __no)
        {
            ...
            try
            {
                ...
                //---cek data name Packing Month            
                string sqlCek = string.Empty;
                sqlCek = "select top 1 id from Dialogs where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                selectCek.CommandTimeout = 0;
                SqlDataReader RDCek = selectCek.ExecuteReader();
                ...
                cmd.CommandText = "insert into DialogDetails (DialogId, SubModule, POS, PV, WW, AA, LK, SC, SP, ST, Part, ZB, Name, Quantity, UQTY, " +
                                    " CodeRule, R, BZA, DevBZA, PEMFrom, PEMTo ) " +
                                    " values (" + IdHeader + ",'" + SubModule + "', '" + POS + "', '" + PV + "','" + WW + "', " +
                                    " '" + AA + "','" + LK + "','" + SC + "','" + SP + "','" + ST + "','" + Part + "'," +
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/Dialogs/list')", true);
            }
        ...
        private void Import_Proses_VO(string strValue, int __no)
        {
            ...
            try
            {
                ...
                string VehicleType2 = VehicleType.Substring(0,6);                
                ...
                //---cek data name Packing Month            
                string sqlCek = string.Empty;
                sqlCek = "select top 1 id from VehicleOrders where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                selectCek.CommandTimeout = 0;
                SqlDataReader RDCek = selectCek.ExecuteReader();
                ...
                //----get variant
                string sqlget = string.Empty;
                sqlget = "select top 1 b.Id VariantId from Models a left join Variants b on a.Id=b.ModelId where a.Baumuster='" + VehicleType2 + "' order by isnull(b.Code,0) desc ";
                SqlCommand selectGet = new SqlCommand(sqlget, conn);
                selectGet.CommandTimeout = 0;
                SqlDataReader RDGet = selectGet.ExecuteReader();
                ...
                if (VHVarianId == 0)
                {
                    string sqlget2 = string.Empty;
                    sqlget2 = "select top 1 b.Id VariantId from Models a left join Variants b on a.Id=b.ModelId where a.Baumuster='" + VehicleType2 + "' order by isnull(b.Code,0) asc ";
                    SqlCommand selectGet2 = new SqlCommand(sqlget2, conn);
                    selectGet2.CommandTimeout = 0;
                    SqlDataReader RDGet2 = selectGet2.ExecuteReader();

                    while (RDGet2.Read())
                    {
                        VHVarianId = Convert.ToInt32(RDGet2["VariantId"]);
                    }
                    RDGet2.Close();
                }
                //-----------------------------end get Variant---

                cmd.CommandText = "insert into VehicleOrderDetails (VehicleOrderId, [Plan], ProdNumber, OrderNumber, VehicleNumber, DeliveryNumber, VehicleType, EngineType, " +
                                    " PlantDispathDate, Interior, Paint, Model, CountryName, EngineNumber, SampleDigit, NumberOfCode, Codes, VariantId ) " +
                                    " values (" + IdHeader + ",'" + Plan + "', '" + ProdNumber + "', '" + OrderNumber + "','" + VehicleNumber + "', " +
                                    " '" + DeliveryNumber + "','" + VehicleType + "','" + EngineType + "','" + PlantDispathDate + "','" + Interior + "','" + Paint + "'," +
                                    " '" + Model + "','" + CountryName + "', '" + EngineNumber + "','" + SampleDigit + "', '" + NumberOfCode + "', '" + Codes + "', " + VHVarianId  + ")";
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/VehicleOrders/list')", true);
            }
        ...
        private void Import_Proses_PS(string strValue, int __no)
        {
            ...
            try
            {
                ...
                //---cek data name Packing Month            
                string sqlCek = string.Empty;
                sqlCek = "select top 1 id from ProductionSequence where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                selectCek.CommandTimeout = 0;
                SqlDataReader RDCek = selectCek.ExecuteReader();
                ...
                cmd.CommandText = "INSERT INTO ProductionSequenceDetail " +
                                    "(ProductionSequenceId,Type,CummFigr,MaterialNo,OrderNo,SerialNumber " +
                                    ",DBProdSIFI,CommnosNumber,Lot,EngineNo,ColCode,IntCode,BomExpl,ChassisNoDCAG,FinishDate)  VALUES " +
                                    " (" + IdHeader + ",'" + Type + "'," + CummFigr + ", " +
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/ProductionSequence/list')", true);
            }
        ...
        private void Import_Proses_Trolley(string strValue, int __no)
        {
            ...
            try
            {
                ...
                //---cek data name Packing Month            
                string sqlCek = string.Empty;
                sqlCek = "select top 1 id from TrolleyLists where PackingMonth=" + mPackingMonthId + " and ModelId=" + mModelId + " and VarianId = " + mVarianId + " ";
                SqlCommand selectCek = new SqlCommand(sqlCek, conn);
                selectCek.CommandTimeout = 0;
                SqlDataReader RDCek = selectCek.ExecuteReader();
                ...
                if (IdHeaderCek == 0)
                {
                    cmd.CommandText = "insert into TrolleyLists (PackingMonth, ModelId, VarianId, FileType) " +
                                " values (" + mPackingMonthId + "," + mModelId + ", " + mVarianId + ", " + mFileType + ")";
                ...
                cmd.CommandText = "INSERT INTO TrolleyListDetail " +
                                    "(TrolleyListId,Model,Varian,Type,Station,Trolley,TrolleyType,TrolleyNo " +
                                    ",Rack,Consignment,WI,Part,[Desc],Quantity,idx,Valid,Package,Component)  VALUES " +
                                    " (" + IdHeader + ",'" + Model + "','" + Varian + "','" + Type + "','" + Station + "','" + Trolley + "','" +
            ...
            {
                Page.ClientScript.RegisterStartupScript(GetType(), "script", "AlertRedirect('/TrolleyLists/list')", true);
            }
        ...
        {
            //GetdataSave_mPackingMonth = Convert.ToInt16(PackingMonth.Value);
            GetdataSave_mModelId = Convert.ToInt16(ModelId.SelectedItem.Value);
            GetdataSave_mVarianId = Convert.ToInt16(VarianId.SelectedItem.Value);
        ...
        private void Process_SaveMaster(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_Alteration(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_Dialog(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_VO(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_PS(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_Trolley(int intModelId, int intVarianId, string intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        private void Process_SaveMaster_AssyPC(int intModelId, int intVarianId, int intPackingMonth, int intFileType)
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["AppDb"].ConnectionString);
            SqlCommand cmd = new SqlCommand();
        ...
        {
            //GetdataSave_mPackingMonth = Convert.ToInt32(dtPackingMonth.Text);
            GetdataSave_mPackingMonth = string.Format("{0:yyyyMM}", dtPackingMonth.Text);
            GetdataSave_mModelId = Convert.ToInt16(ModelId.SelectedItem.Value);
            //GetdataSave_mVarianId = Convert.ToInt16(VarianId.SelectedItem.Value);
            ///arf: varianid.selecteditemnull, jadi giniin dulu sementara
            GetdataSave_mVarianId = Convert.ToInt16(Request.QueryString["variant"]);  
            ...
            #region tidak pake table packing month
            //---get data name Packing Month            
            //string sqlPM = string.Empty;
            ...
            sqlMD = "SELECT m.ModelName from Models m WHERE m.Id =" + GetdataSave_mModelId + " ";
            ...
            sqlVR = "SELECT v.Variant FROM Variants v where v.Id = " + GetdataSave_mVarianId + " ";
        ...
        protected void ModelId_SelectedIndexChanged(object sender, EventArgs e)
        {
